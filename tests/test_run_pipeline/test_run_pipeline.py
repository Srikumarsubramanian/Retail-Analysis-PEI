...
from retail_analysis.databricks.notebooks import run_pipeline as pipe
...
def pipeline_setup(monkeypatch):
    ...
    monkeypatch.setattr(
        "retail_analysis.databricks.notebooks.ingestion.run_bronze", bronze
    )
    monkeypatch.setattr(
        "retail_analysis.databricks.notebooks.enrich_customers_products.run_silver", silver
    )
    monkeypatch.setattr(
        "retail_analysis.databricks.notebooks.master_orders.run_master_orders", master
    )
    monkeypatch.setattr(
        "retail_analysis.databricks.notebooks.aggregate.run_profit_aggregate", aggregate
    )
    monkeypatch.setattr(
        "retail_analysis.databricks.notebooks.kpis.run_reporting", reporting
    )
