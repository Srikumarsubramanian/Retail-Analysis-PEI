...
from retail_analysis.databricks.notebooks.silver.create_enriched_orders import build_master_orders 
from retail_analysis.templates.schema import ORDERS_SCHEMA, CUSTOMER_SCHEMA, PRODUCTS_SCHEMA
from datetime import date


import retail_analysis.databricks.notebooks.silver.create_enriched_orders as module

from retail_analysis.databricks.utils.util_funcs.util import read_data, merge_delta_upsert, enforce_schema
...
def test_total_revenue_uses_quantity(spark, customers_df, products_df, qty, price, discount, expected):
    ...
    result = module.build_master_orders(spark, orders, customers_df, products_df)
    row = get_row(result)
...
def test_left_join_missing_dimension_data(spark):
    ...
    row = get_row(module.build_master_orders(spark, orders, customers, products))
...
def test_output_schema_and_columns(spark, customers_df, products_df):
    ...
    df = module.build_master_orders(spark, orders, customers_df, products_df)
...
def test_updated_at_column_exists_and_not_null(spark, customers_df, products_df):
    ...
    row = get_row(module.build_master_orders(spark, orders, customers_df, products_df))
