...
import retail_analysis.databricks.notebooks.master_orders as m
...
@pytest.mark.gold
def test_prepare_orders_rounds_profit(spark):
    """
    Ensures profit is aggregated and rounded to 2 decimal places.
    """
    df = spark.createDataFrame([
        (1, "2024-01-01", "2024-01-02", "Standard", 1, 101, 0.0, 1, 100, 10.123),
        (1, "2024-01-01", "2024-01-02", "Standard", 1, 101, 0.0, 1, 100, 0.456),
    ], [
        "order_id","order_date","ship_date","ship_mode",
        "customer_id","product_id","discount",
        "quantity","price","profit"
    ...
    row = m._prepare_orders(df).first()

    assert row["profit"] == pytest.approx(10.58, rel=1e-2)
